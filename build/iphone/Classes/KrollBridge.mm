...
@implementation HowIsTheWeatherObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_howistheweather);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any HowIsTheWeather patches in JS before a context is loaded... nice for 
...
{
	// create HowIsTheWeather global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_howistheweather = [[HowIsTheWeatherObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_howistheweather];
	
	NSString *_howistheweatherNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_howistheweatherNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_howistheweather valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.howistheweather.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
